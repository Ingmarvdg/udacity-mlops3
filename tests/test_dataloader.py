...
def test_trainval_data(trainval_data_path: str):
    test_size = 0.3
    train_x, test_x, train_y, test_y = dataloader.load_trainval_data(trainval_data_path, test_size)

    assert len(train_x.columns) == EXPECTED_COLUMNS
    assert len(test_x.columns) == EXPECTED_COLUMNS
    assert np.isclose(len(test_y) / (len(train_y) + len(test_y)), test_size, atol=0.02)
